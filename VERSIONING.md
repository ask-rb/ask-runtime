# Versioning — ask-runtime

This file is this repository's **canonical versioning policy**. When
anything else in the repo (README, RELEASE.md, GOAL.md, commit messages)
...
Patch numbers advance by exactly one per release: `0.1.0` → `0.1.1` →
`0.1.2`. Never skip or jump patch numbers (`0.1.0` → `0.1.3` is wrong), even
...
gemchain guard ask-runtime
gemchain update ask-runtime <new-version> --dry-run
gemchain update ask-runtime <new-version>
```
...
3. Build passes — `bundle exec rake build` (or
   `gem build ask-runtime.gemspec`).
4. Changelog — `Unreleased` entries moved under the new `X.Y.Z` heading with
   the release date.
5. Version — bumped in `lib/ask/runtime/version.rb` by gemchain, not by hand.
...
| `lib/ask/runtime/version.rb` at HEAD | `X.Y.Z` |
| Git tag | `vX.Y.Z` |
...
- Several bug fixes ship together: one release, `0.1.0` → `0.1.1` (never
  `0.1.3`).
- New backwards-compatible API: `0.1.1` → `0.2.0`.
...
- This gem bumped because a dependency released: patch only, e.g. `0.1.1` →
  `0.1.2`, via the gemchain cascade — never hand-published.
